    ...
    private async void processUpdate(Update update)
    {
        ...
        switch (update.Type)
        {
            case UpdateType.Message:
                ...
                if (!Directory.Exists("DownloadFiles")) Directory.CreateDirectory("DownloadFiles");
                switch (text)
                {
                    case "/start":
                        ...
                        break;
                    case "Список файлов":
                        var files = Directory.GetFiles("DownloadFiles");
                        var count = 1;
                        if (files.Length > 0)
                            ...
                            await _client.SendTextMessageAsync(update.Message.Chat.Id,
                                "Нет файлов для загрузки.", replyMarkup: GetButtons());
            ...
            case UpdateType.CallbackQuery:
                ...
                catch (Exception e)
                {
                    await _client.SendTextMessageAsync(update.CallbackQuery.Message.Chat.Id,
                        "Нет такого файла. Возможно он был перемещен или удален.", replyMarkup: GetButtons());
                }
